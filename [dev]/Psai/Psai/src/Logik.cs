...
namespace psai.net
{
    ...
    {
        private static readonly string PSAI_VERSION = ".NET 1.7.3";
        ...
        int m_targetVoice;                  // upcoming output channel number
		...
		Theme m_effectiveTheme;						    /* the Theme currently playing, or, if a Theme change has been initiated by calling PlayThemeNowOrAtEnd..(), effectiveTheme refers to that next Theme. Stays valid in rest mode. */	
        ...
        /* This is used to store calls to triggerMusicTheme, that will cause a transition at the end of the current Segment. 
         * We don't want each trigger to be evaluated instantly (calculating the ideal Segment to play next), as further incoming trigger calls would 
         * replace that evaluation, wasting processing power. Instead, we evaluate the last nonInterruptingTrigger in the SegmentEndApproachingHandler().
         */
        ThemeQueueEntry m_latestEndOfSegmentTriggerCall = new ThemeQueueEntry();
        ...
        internal Logik()
        {
            ...
            for (int i = 0; i < PSAI_CHANNEL_COUNT; i++)
            {
                ...
                {
                    m_playbackChannels[i] = new PlaybackChannel(this, true);
                }
                else
                {
                    m_playbackChannels[i] = new PlaybackChannel(this, false);
                }                
            }
        ...
        /** returns the id of the theme set as baseTheme, or -1 if none has been set yet.
	    */
        ...
        {
            Theme lastBasicMood = m_soundtrack.getThemeById(themeId);
            if (lastBasicMood != null)
            {
                SetThemeAsLastBasicMood(lastBasicMood);
            }
            else
            {
		    ...
		    if (voiceId > -1)
		    {
                ...
                for (int i = 0; i < m_fadeVoices.Count; i++) {
                    ...
                    if (fadeData.voiceNumber == voiceId) {
                        ...
                        fadeData.fadeoutDeltaVolumePerUpdate = currentVol / (fadeoutMillis / (float)Logik.PSAI_FADING_UPDATE_INVERVAL_MILLIS);
                        fadeData.currentVolume = currentVol;
                        return;
                    }
                }		    
	    ...
	    */
        ...
        internal PsaiResult setPaused(bool setPaused)
        {
            ...
            if ((setPaused && !m_paused) || (!setPaused && m_paused))
            {
                ...
                else
                {
                    ...
                    m_estimatedTimestampOfTargetSnippetPlayback += pausedPeriod;
                    ...
                    #if !(PSAI_NOLOG)
                    {
                        ...
                        {
                            StringBuilder sb = new StringBuilder();
                            sb.Append("pausePeriod=");
                            sb.Append(pausedPeriod);
                            sb.Append(" m_estimatedTimestampOfTargetSnippetPlayback=");
                            sb.Append(m_estimatedTimestampOfTargetSnippetPlayback);
                            Logger.Instance.Log(sb.ToString(), LogLevel.debug);
                        }
                    }
            ...
            else
            {
                #if !(PSAI_NOLOG)
                {
                    ...
                    {
                        StringBuilder sb = new StringBuilder();
                        sb.Append("setPaused() has been ignored, as psai was already in the given state.");
                        sb.Append(" m_paused=" );
                        sb.Append(m_paused);
                        sb.Append(" setPaused=");
                        sb.Append(setPaused);
			    ...
			    switch(m_psaiState)
			    {
                    case PsaiState.playing:
				    {
						...
						if (m_currentSegmentPlaying != null && m_effectiveTheme.themeType != ThemeType.basicMood)
						{
							...
							{
								PlayThemeNowOrAtEndOfCurrentSegmentAndStartEvaluation(GetLastBasicMoodId(), m_lastBasicMood.intensityAfterRest, m_lastBasicMood.musicDurationGeneral, true, false);
							}
							else
							{		
        ...
        internal PsaiResult TriggerMusicTheme(int argThemeId, float argIntensity)
        {
            ...
            if (argTheme == null)
            {
                #if !(PSAI_NOLOG)
                {
                    ...
                    {
                    	Logger.Instance.Log(string.Format("TriggerMusicTheme() - Theme not found ! themeId={0}", argThemeId), LogLevel.errors);
                    }
                }
                #endif
        ...
        internal PsaiResult TriggerMusicTheme(Theme argTheme, float argIntensity, int argMusicDuration)
        {
		    ...
		    else if (m_psaiPlayMode == PsaiPlayMode.cutScene && m_psaiStateIntended == PsaiState.silence && m_currentSegmentPlaying != null)
		    {
                ...
                return PlayThemeNowOrAtEndOfCurrentSegmentAndStartEvaluation(argTheme.id, argIntensity, argMusicDuration, true, false);						
		    }
            ...
            {
                ThemeInterruptionBehavior tib = Theme.GetThemeInterruptionBehavior(effectiveTheme.themeType, argTheme.themeType);

                if (tib == ThemeInterruptionBehavior.at_end_of_current_snippet || tib == ThemeInterruptionBehavior.never)
                {
                    m_psaiStateIntended = PsaiState.playing;
                    return PlayThemeNowOrAtEndOfCurrentSegmentAndStartEvaluation(argTheme.id, argIntensity, argMusicDuration, false, false);
                }
            }
		    ...
		    if (effectiveSegment.ThemeId == argTheme.id)
		    {
                ...
                m_latestEndOfSegmentTriggerCall.themeId = -1;		// clear any non-interrupting trigger call we may have received recently!  
            ...
            {
                //
                //  a Basic Mood has been triggerd...
                //                                                                    
                case ThemeType.basicMood:
                    {
                        switch (effectiveTheme.themeType)
                        {
                            // ... while a Basic Mood is playing:               Transition at end of Segment.   No need to clear the queue or queue anything.
                            case ThemeType.basicMood:
                                return PlayThemeAtEndOfCurrentSegment(argTheme, argIntensity, argMusicDuration);

                            // ...while a Basic Mood Alteration is playing:     Tansition at end of Theme.      No need to clear the queue or to queue anything, as a BasicMoodAlt will always transition directly to the LastBasicMood afterwards.
                            case ThemeType.basicMoodAlt:                            
                                return PsaiResult.OK; 

                            // ...while a DramaticEvent is playing:             Transition at end of Theme.     No need to clear the queue or to queue anything, as a DramaticEvent will always transition directly to the LastBasicMood afterwards.
                            case ThemeType.dramaticEvent:                            
                                return PsaiResult.OK; 

                            // ...while an Action Theme is playing:             No direct transition. Psai will go to Rest and wake up with the new Last Basic Mood (already set).
                            case ThemeType.action:                            
                                return PsaiResult.OK; 

                            // ...while a Shock is playing:                     No direct transition. Don't touch the Queue, as there maybe already an Action Theme queued.
                ...
                //
                //  a Basic Mood Alteration has been triggerd...
                //                                                                    
                case ThemeType.basicMoodAlt:
                    {
                        switch (effectiveTheme.themeType)
                        {
                            // ... while a Basic Mood is playing:               Transition at end of Segment.   The Queue should be empty.
                            case ThemeType.basicMood:
                                return PlayThemeAtEndOfCurrentSegment(argTheme, argIntensity, argMusicDuration);

                            // ...while a Basic Mood Alteration is playing:     Tansition at end of Segment.    No need to clear the queue or to queue anything, as a BasicMoodAlt will always transition directly to the LastBasicMood afterwards.
                            case ThemeType.basicMoodAlt:                            
                                return PlayThemeAtEndOfCurrentSegment(argTheme, argIntensity, argMusicDuration);

                            // ...while a DramaticEvent is playing:             Transition at end of Segment.     No need to clear the queue or to queue anything, as a DramaticEvent will always transition directly to the LastBasicMood afterwards.
                            case ThemeType.dramaticEvent:                            
                                return PlayThemeAtEndOfCurrentSegment(argTheme, argIntensity, argMusicDuration);

                            // ...while an Action Theme is playing:             The trigger will be ignored, as Action has the higher priority.
                            case ThemeType.action:                            
                                return PsaiResult.triggerIgnoredLowPriority; 

                            // ...while a Shock is playing:                     Transition at end of Segment, as long as no Action is queued.
                            case ThemeType.shock:
                                {
                                    ThemeType followingThemeType = getThemeTypeOfFirstThemeQueueEntry();
                                    if (followingThemeType == ThemeType.action)
                                    {
                                        return PsaiResult.triggerIgnoredLowPriority;
                                    }
                ...
                //
                //  a Dramatic Event has been triggered...
                //                                                                    
                case ThemeType.dramaticEvent:
                    {
                        ...
                        switch (effectiveTheme.themeType)
                        {
                            // ... while a Basic Mood is playing:               Transition immediately. The queue should be empty.
                            case ThemeType.basicMood:
                                return PlayThemeNowOrAtEndOfCurrentSegmentAndStartEvaluation(argTheme.id, argIntensity, argMusicDuration, true, false);

                            // ...while a Basic Mood Alteration is playing:     Transition immediately. The queue should be empty.
                            case ThemeType.basicMoodAlt:                            
                                return PlayThemeNowOrAtEndOfCurrentSegmentAndStartEvaluation(argTheme.id, argIntensity, argMusicDuration, true, false);

                            // ...while a DramaticEvent is playing:             Transition at end of Segment. The queue should be empty.
                            case ThemeType.dramaticEvent:                            
                                return PlayThemeAtEndOfCurrentSegment(argTheme, argIntensity, argMusicDuration);
                ...
                //
                //  an Action Theme has been triggered...
                //                                                                    
                case ThemeType.action:
                    {
                        switch (effectiveTheme.themeType)
                        {
                            // ... while a Basic Mood is playing:               Play immediately. Don't queue anything. The Queue should be empty, but clear the latestEndOfSegmentTriggerCall, as there might have been a trigger to BasicMoodAlt, which we don't want to return to.
                            case ThemeType.basicMood:
                                {
                            ...
                            // ...while a Basic Mood Alteration is playing:     Play immediately. Don't queue anything. The Queue should be empty, but clear the latestEndOfSegmentTriggerCall, as there might have been a trigger to BasicMoodAlt, which we don't want to return to.
                            case ThemeType.basicMoodAlt:                            
                                {
                                    ClearLatestEndOfSegmentTriggerCall();
                                    return PlayThemeNowOrAtEndOfCurrentSegmentAndStartEvaluation(argTheme.id, argIntensity, argMusicDuration, true, false);
                            ...
                            // ...while a DramaticEvent is playing:             Play immediately. Don't queue anything. The Queue should be empty, but clear the latestEndOfSegmentTriggerCall, as there might have been a trigger to BasicMoodAlt, which we don't want to return to.
                            case ThemeType.dramaticEvent:                            
                                {
                                    ClearLatestEndOfSegmentTriggerCall();
                                    return PlayThemeNowOrAtEndOfCurrentSegmentAndStartEvaluation(argTheme.id, argIntensity, argMusicDuration, true, false);
                            ...
                            // ...while an Action Theme is playing:             Transition at end of Segment. Don't queue anything. The Queue should be empty, as Action always returns to Rest Mode. Don't clear the latestEndOfSegmentTriggerCall, as otherwise it would be rendered useless.
                            case ThemeType.action:                            
                                {                            
                                    return PlayThemeAtEndOfCurrentSegment(argTheme, argIntensity, argMusicDuration);
                                }
                                
                            // ...while a Shock is playing:                     Transition at end of Theme. Override the queued Theme in any case.
                            case ThemeType.shock:
                                {
                                    return PlayThemeAtEndOfCurrentTheme(argTheme, argIntensity, argMusicDuration);          
                                }                                
                ...
                case ThemeType.shock:
                    {
                        ...
                        switch (effectiveTheme.themeType)
                        {
                            // ... while a Basic Mood is playing:               Play immediately. Don't queue anything, as we want the music to go to Rest afterwards, if no more triggers are incoming.
                            case ThemeType.basicMood:
                                {
                                    return PlayThemeNowOrAtEndOfCurrentSegmentAndStartEvaluation(argTheme.id, argIntensity, argMusicDuration, true, false);
                                }                                

                            // ...while a Basic Mood Alteration is playing:     Play immediately. Don't queue anything, as we want the music to go to Rest afterwards, if no more triggers are incoming.
                            case ThemeType.basicMoodAlt:                            
                                {
                                    return PlayThemeNowOrAtEndOfCurrentSegmentAndStartEvaluation(argTheme.id, argIntensity, argMusicDuration, true, false);
                                }                                

                            // ...while a DramaticEvent is playing:             Play immediately. Don't queue anything, as we want the music to go to Rest afterwards, if no more triggers are incoming.
                            case ThemeType.dramaticEvent:                            
                                {
                                    return PlayThemeNowOrAtEndOfCurrentSegmentAndStartEvaluation(argTheme.id, argIntensity, argMusicDuration, true, false);
                                }                                

                            // ...while an Action Theme is playing:             Play immediately. Return to the interrupted Action Theme directly afterwards.
                            case ThemeType.action:                            
                                {
                                    ClearQueuedTheme();
                            ...
                            // ...while a Shock is playing:                     Play at end of Segment. Don't touch the queue, as we still want to return to an Action Theme that might have been interrupted.
                            case ThemeType.shock:
                                {
                                    return PlayThemeAtEndOfCurrentSegment(argTheme, argIntensity, argMusicDuration);
                                }                                
	    ...
	    */
        ...
        void ClearLatestEndOfSegmentTriggerCall()
        {
            this.m_latestEndOfSegmentTriggerCall.themeId = -1;
        }


        void ClearQueuedTheme()
        {            
            m_themeQueue.Clear();
        }
	    ...
	    bool pushThemeToThemeQueue(int themeId, float intensity, int musicDuration, bool clearThemeQueue, int restTimeMillis, PsaiPlayMode playMode, bool holdIntensity)
	    {
		    ...
		    if (theme != null)
		    {
			    ...
			    #if !(PSAI_NOLOG)
			    {
                    ...
                    {
	                    StringBuilder sb = new StringBuilder();
	                    sb.Append(" m_themeQueue.size() [before] =");
	    ...
	    */
	    PsaiResult PlaySegment(Segment targetSnippet, bool immediately)
	    {
            ...
            if(psaiResult != PsaiResult.OK) 
			{
                #if !(PSAI_NOLOG)
			    {
			        string debugLog = string.Format("LoadSegment failed. {0} was the current segment {1} was the theme",
			            m_currentSegmentPlaying == null ? "null" : m_currentSegmentPlaying.Name,
			            m_effectiveTheme == null ? "null" : m_effectiveTheme.Name);
                	Logger.Instance.Log(debugLog, LogLevel.errors);
				}
				#endif
                return psaiResult;
            }	
		    ...
		    {			
			    ...
			    if (millisUntilNextSnippetPlayback > s_audioLayerMaximumLatencyForPlayingBackUnbufferedSounds)
			    {
                    ...
                    #if !(PSAI_NOLOG)
                    {
                        ...
                        {
                            StringBuilder sb = new StringBuilder();
                            sb.Append("m_timerStartSnippetPlayback set to ");
                            sb.Append(millisUntilNextSnippetPlayback);
                            sb.Append("  m_estimatedTimestampOfTargetSnippetPlayback set to ");
                            sb.Append(m_estimatedTimestampOfTargetSnippetPlayback);
                            sb.Append("  millisElapsed after current snippet playcall=");
                            sb.Append(millisElapsed);
                            Logger.Instance.Log(sb.ToString(), LogLevel.debug);
                        }
                    }
                    #endif
	    ...
	    void PlayTargetSegmentImmediately()
	    {
            ...
            SetSegmentEndApproachingAndReachedTimersAfterPlaybackHasStarted(snippetPlaybackDelayMillis);
        ...
        internal void SetSegmentEndApproachingAndReachedTimersAfterPlaybackHasStarted(int snippetPlaybackDelayMillis)
        {
            ...
            int millisUntilUpcomingSnippetEnd = m_currentSegmentPlaying.audioData.GetFullLengthInMilliseconds() + snippetPlaybackDelayMillis;
            int millisUntilNextCalculateCall = (int)(millisUntilUpcomingSnippetEnd
                - m_currentSegmentPlaying.audioData.GetPostBeatZoneInMilliseconds()
                - m_currentSegmentPlaying.MaxPreBeatMsOfCompatibleSnippetsWithinSameTheme
            ...
            if (millisUntilNextCalculateCall < 0)
            {
                #if !(PSAI_NOLOG)
                {
                    if (LogLevel.warnings <= Logger.Instance.LogLevel)
                    {
                        ...
                        sb.Append(millisUntilNextCalculateCall);
                        sb.Append(").");
                        sb.Append(" This means that either 1.) Your playback and buffer latency settings are set too high (in Unity: see PsaiCoreManager) , or 2.) The main region of Segment '");
                        sb.Append(m_currentSegmentPlaying.Name);
                        sb.Append("' (ThemeId=");
                        sb.Append(m_currentSegmentPlaying.ThemeId);
                        sb.Append(") is too short, or 3.) The Prebeat region of at least one of its compatible Segments is too long. ");
	    ...
	    PsaiResult PlayThemeNowOrAtEndOfCurrentSegmentAndStartEvaluation(ThemeQueueEntry tqe, bool immediately)
	    {
	    ...
	    *		
	    */
	    PsaiResult PlayThemeNowOrAtEndOfCurrentSegmentAndStartEvaluation(int themeId, float intensity, int musicDuration, bool immediately, bool holdIntensity)
	    {
		    #if !(PSAI_NOLOG)
		    {
                if (LogLevel.debug <= Logger.Instance.LogLevel)
        ...
        {
            return StopMusic(immediately, (int)(PSAI_FADEOUTMILLIS_STOPMUSIC));
        }
        ...
        internal PsaiResult StopMusic(bool immediately, int fadeOutMilliSeconds)
	    {
		    ...
		    #if !(PSAI_NOLOG)
            {
                if (LogLevel.info <= Logger.Instance.LogLevel)
                {
                    ...
                    if (immediately)
                    {
                        sb.Append("  fadeout-time (ms): " + fadeOutMilliSeconds);
                    }
            ...
            if (immediately && fadeOutMilliSeconds <=0)
            {
                ...
                #if !(PSAI_NOLOG)
                {
                    ...
                    {
                        StringBuilder sb = new StringBuilder();
                        sb.Append("invalid argument: the StopMusic fadeout-time was negative, and thus has been set to the default value of ");
                        sb.Append(fadeOutMilliSeconds);
                        sb.Append(" ms.");
                        Logger.Instance.Log(sb.ToString(), LogLevel.warnings);
                    }
                }
        ...
        void WriteLogWarningIfThereIsNoDirectPathForEffectiveSnippetToEndSnippet()
	    {
		    ...
		    if (LogLevel.warnings <= Logger.Instance.LogLevel)
		    {
			    ...
			    {
	                StringBuilder sb = new StringBuilder();
	                sb.Append("There is no direct path to an END-Segment from the current Segment, thus psai will play an indirection via Segment: ");
	                sb.Append(effectiveSnippet.nextSnippetToShortestEndSequence.Name);
	                sb.Append(" Please add a direct compatible Transition to an END-Segment for Segment ");
	                sb.Append(effectiveSnippet.Name);
	                sb.Append(" to have psai end the Theme more quickly.");
	                Logger.Instance.Log(sb.ToString(), LogLevel.warnings);
			    }
		    }
		    #endif
        ...
        void SegmentEndApproachingHandler()
	    {
		    ...
		    #if !(PSAI_NOLOG)
		    {
                if (LogLevel.debug <= Logger.Instance.LogLevel)
                {
	                ...
	                sb.Append(m_latestEndOfSegmentTriggerCall.themeId);
	                Logger.Instance.Log(sb.ToString(), LogLevel.debug);
                }
		    }
		    #endif
			    ...
			    default:
				    {
					    ...
					    if (m_returnToLastBasicMoodFlag)
					    {
						    ...
						    {
							    PlayThemeNowOrAtEndOfCurrentSegmentAndStartEvaluation(GetLastBasicMoodId(), m_lastBasicMood.intensityAfterRest, m_lastBasicMood.musicDurationGeneral, false, false);
							    m_returnToLastBasicMoodFlag = false;
							    return;
						    }
					    }
                        ...
                        if (m_psaiPlayMode == PsaiPlayMode.regular && m_latestEndOfSegmentTriggerCall.themeId != -1)
                        {

                            Theme nonInterruptingTheme = m_soundtrack.getThemeById(m_latestEndOfSegmentTriggerCall.themeId);

                            if (m_currentSegmentPlaying.CheckIfAtLeastOneDirectTransitionOrLayeringIsPossible(m_soundtrack, nonInterruptingTheme.id))
                            {
                                PlayThemeNowOrAtEndOfCurrentSegmentAndStartEvaluation(m_latestEndOfSegmentTriggerCall.themeId, m_latestEndOfSegmentTriggerCall.startIntensity, m_latestEndOfSegmentTriggerCall.musicDuration, false, m_latestEndOfSegmentTriggerCall.holdIntensity);
                                m_latestEndOfSegmentTriggerCall.themeId = -1;
                            }
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    PlayThemeNowOrAtEndOfCurrentSegmentAndStartEvaluation(m_latestEndOfSegmentTriggerCall.themeId, m_latestEndOfSegmentTriggerCall.startIntensity, m_latestEndOfSegmentTriggerCall.musicDuration, true, m_latestEndOfSegmentTriggerCall.holdIntensity);
                                    m_latestEndOfSegmentTriggerCall.themeId = -1;
                                }
                            }
                        }
        ...
        /// <summary>
        /// Stores an incoming trigger call which that will cause psai to switch the Theme at the end of the current Segment for later evaluation.
        /// </summary>
        /// <param name="argTheme"></param>
        /// <param name="intensity"></param>
        /// <param name="musicDuration"></param>
        /// <returns></returns>
        PsaiResult PlayThemeAtEndOfCurrentSegment(Theme argTheme, float intensity, int musicDuration)
        {
            #if !(PSAI_NOLOG)
            {            
                if (LogLevel.debug <= Logger.Instance.LogLevel)
                {
	                StringBuilder sb = new StringBuilder();
                    sb.Append("PlayThemeAtEndOfCurrentSegment() argTheme=");
	                sb.Append(argTheme.Name);
	                sb.Append("  intensity=");
	                sb.Append(intensity);
	                sb.Append("  musicDuration=");
        ...
        PsaiResult PlayThemeAtEndOfCurrentTheme(Theme argTheme, float argIntensity, int argMusicDuration)
        {
            ClearLatestEndOfSegmentTriggerCall();
            ClearQueuedTheme();
            pushThemeToThemeQueue(argTheme.id, argIntensity, argMusicDuration, false, 1, PsaiPlayMode.regular, false);
            return PsaiResult.OK;
        }
        ...
        void InitiateTransitionToRestOrSilence(PsaiState psaiStateIntended)
        {
			    ...
			    case ThemeType.dramaticEvent:
                    {
                        if (m_lastBasicMood != null)
                        {
                            PlayThemeNowOrAtEndOfCurrentSegmentAndStartEvaluation(m_lastBasicMood.id, m_lastBasicMood.intensityAfterRest, m_lastBasicMood.musicDurationAfterRest, false, false);
                        } 
                        else
                        {
	    ...
	    */
        ...
        void WakeUpFromRestHandler()
	    {
		    ...
		    if (m_effectiveTheme != null)
		    {
			    PlayThemeNowOrAtEndOfCurrentSegmentAndStartEvaluation(m_effectiveTheme.id, m_effectiveTheme.intensityAfterRest, m_effectiveTheme.musicDurationAfterRest, true, false);			
	    ...
	    */
        ...
        internal PsaiResult MenuModeEnter(int menuThemeId, float menuIntensity)
	    {
		    ...
		    if (m_psaiPlayMode != PsaiPlayMode.menuMode)
		    {
			    ...
			    if (menuTheme != null)
			    {
				    PlayThemeNowOrAtEndOfCurrentSegmentAndStartEvaluation(menuThemeId, menuIntensity, 666, true, true);
			    }
			    else
			    {
                    setPaused(false);
                    ...
                    #if !(PSAI_NOLOG)
                    {
                        ...
                        {
                            StringBuilder sb = new StringBuilder();
                            sb.Append("No Menu Theme has been set -> pausing");
                            Logger.Instance.Log(sb.ToString(), LogLevel.warnings);
                        }
                    }
                    #endif
	    ...
	    internal PsaiResult CutSceneEnter(int themeId, float intensity)
	    {
		    ...
		    switch(m_psaiPlayModeIntended)		// we use 'intended' here and not m_psaiPlayMode, so immediate switches back to Cutscene after leaving will be ignored and will not cause trouble
		    {
		    case PsaiPlayMode.cutScene:
			    {
				    #if !(PSAI_NOLOG)
		    ...
		    default:
			    {
				    ...
				    {
					    PlayThemeNowOrAtEndOfCurrentSegmentAndStartEvaluation(themeId, intensity, cutSceneTheme.musicDurationGeneral, true, true);
					    return PsaiResult.OK;
				    }
				    else
				    {									
					    PlayThemeNowOrAtEndOfCurrentSegmentAndStartEvaluation(m_lastBasicMood.id, intensity, m_lastBasicMood.musicDurationGeneral, true, true);
					    return PsaiResult.unknown_theme;
				    }				
	    ...
	    // Used internally for entering Menu Mode and Cut Scenes
	    void PushEffectiveThemeToThemeQueue(PsaiPlayMode playModeToReturnTo)
	    {
            ...
            if (m_latestEndOfSegmentTriggerCall.themeId != -1)
            {
                ...
                Theme nonInterruptingTheme = m_soundtrack.getThemeById(m_latestEndOfSegmentTriggerCall.themeId);
                pushThemeToThemeQueue(nonInterruptingTheme.id, m_latestEndOfSegmentTriggerCall.startIntensity, m_latestEndOfSegmentTriggerCall.musicDuration, false, 0, PsaiPlayMode.regular, false);
            ...
            else
            {
                #if !(PSAI_NOLOG)
                {
                    ...
                    {
	                    StringBuilder sb = new StringBuilder();
	                    sb.Append("not pushing anything to themeQueue. effectiveSegment=");
	    ...
	    void PopAndPlayNextFollowingTheme(bool immediately)
	    {
		    ...
		    if (getFollowingThemeQueueEntry() != null)
		    {
                ...
                #if !(PSAI_NOLOG)
                {
                    ...
                    {
                        StringBuilder sb = new StringBuilder();
                        sb.Append("first ThemeQueueEntry: ");
                        sb.Append(tqe.ToString());
                        Logger.Instance.Log(sb.ToString(), LogLevel.debug);
                    }
                }
                #endif
			    ...
			    case PsaiPlayMode.regular:
				    {
					    ...
					    {
						    PlayThemeNowOrAtEndOfCurrentSegmentAndStartEvaluation(tqe, immediately);
					    }
                        /*
					    else                        
					    {
			    ...
			    case PsaiPlayMode.cutScene:
				    {
					    PlayThemeNowOrAtEndOfCurrentSegmentAndStartEvaluation(tqe.themeId, tqe.startIntensity, tqe.musicDuration, immediately, tqe.holdIntensity);
				    }
				    break;

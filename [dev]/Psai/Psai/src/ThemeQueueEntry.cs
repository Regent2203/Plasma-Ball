...
namespace psai.net
{
	...
	internal class ThemeQueueEntry : System.ICloneable
	{
        ...
        {
            string result = string.Format("playmode:{0}  themeId:{1}  startIntensity:{2}  restTimeMillis:{3}  holdIntensity:{4}  musicDuration:{5}", playmode, themeId, startIntensity, restTimeMillis, holdIntensity, musicDuration);
            return result;
        }
	};
}
